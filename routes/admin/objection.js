...
router.get('/topics', authorize, function (req, res) {
    ...
    pool.query(sql, (err, results) => {
        ...
        let json = {
            success: true,
            message: 'Get objections successfully',
            data: {
...
router.post('/topics', authorize, function (req, res) {
    ...
    const sql = `INSERT INTO tb_objection_handler SET ?`;
...
router.delete('/topics/:id', authorize, function (req, res) {
    const token = req.headers['authorization'].split(' ')[1]
    var decoded = jwt.decode(token, { complete: true });
    const userId = decoded.payload.id;
    ...
    let body = req.body;
    let sql = `UPDATE tb_objection_handler SET
    is_deleted = 1,
    updated_date = NOW(),
    updated_by = ?
    WHERE 1 = 1
...
router.get('/topics/:id', authorize, function (req, res) {
    const id = req.params.id;

    const sql = `SELECT id, journal_id, title, description, is_enabled
    FROM tb_objection_handler
    WHERE 1 = 1
    AND id = '${id}'
    AND is_deleted = 0`;
...
router.patch('/topics/:id', authorize, async function (req, res) {
    const token = req.headers['authorization'].split(' ')[1]
    var decoded = jwt.decode(token, { complete: true });
    const userId = decoded.payload.id;
